...
1. 电机控制示例代码：
    ```
    ./src/livelybot_bringup/src/motor_rerun.cpp
    ```
...
* 当机器人跌倒时，可以执行跌倒保护功能（自动关闭电机供电），防止机器人损坏或者对周围的环境造成伤害。

1. 执行`livelybo_bringup/launch/robot_falldown_protection_xxx.launch`文件来启动跌倒保护功能

```
roslaunch livelybot_bringup robot_falldown_protection_xxx.launch # xxx为不同的算法和机器人
```

2. 配置跌倒保护功能，在`livelybot_bringup/cfg/falldown_condition_lr_hi.yaml`文件中配置跌倒保护功能配置参数

    其中一个重要的参数是`rotable`,它表示IMU在机器人装配中是否旋转，如果IMU在机器人装配中旋转，则需要将此参数设置为true，否则设置为false。

3. 跌倒保护功能通过机器人姿态数据来判断机器人是否跌倒，通过发布功率板的话题`/power_switch_control`来控制电机的供电开关，因此需要启动IMU节点和功率板节点。启动方法如下：
```
roslaunch yesense_imu run_without_rviz.launch
rosrun livelybot_power power_node
```

### 十三、低电量警告

1. 执行`livelybo_bringup/launch/low_power_warning.launch`文件启动低电量警告功能，当电池电量低于`low_power_warning_voltage`参数指定的值时，会触发手柄振动；

2. 在`livelybo_bringup/launch/low_power_warning.launch`文件中可以配置低电量警告功能配置参数，使能，振动强度，振动时长，低电量阈值等参数；

3. 启动低电量警告功能需要依赖功率板节点发出的电池电量数据，因此需要启动功率板节点，启动方法如下：
```
rosrun livelybot_power power_node
```

### 十四、急停
1. 执行`livelybo_bringup/launch/robot_emergency_stop.launch`文件启动急停功能，此时，按下组合（LT+RT+POWER）键，会触发急停功能，关闭电机供电，启动方法如下：
```
roslaunch livelybot_bringup robot_emergency_stop.launch
```